...
function testIfEqual(a, b) {
	if(typeof a == "object" && typeof b == "object" && a !== null && b !== null) {
		...
		for(var k in a) {
			if(!testIfEqual(a[k], b[k])) return false;
		}
		return true;
	}
	return a == b;
...
function describe(what, callback) {
    DESCRIBES.push(what);
...
function it(what, callback) {
    test(DESCRIBES.join(" - "), callback);
}
...
function expect(obj) {
	return {
		...
		toEqual : function(what) {
		    assert(testIfEqual(obj, what), "expected equal");
		},
		toBeTruthy : function(what) {
		    assert(obj, "expected (" + typeof(obj) + ")" + obj + " to be truthy");
		},
		toBeFalsy : function(what) {
		    assert(!obj, "expected (" + typeof(obj) + ")" + obj + " to be falsy");
		},
		toBeNull: function() {
		    assert(obj === null, "expected null");
		},
		not : {
			toBe : function(what) { assert(obj !== what); },
			toBeTruthy : function(what) { assert(!obj, "expected (" + typeof(obj) + ")" + obj + " to not be truthy"); },
			toEqual : function(what) { assert(!testIfEqual(obj, what), "expected not equal"); }
		} 
	};
}
